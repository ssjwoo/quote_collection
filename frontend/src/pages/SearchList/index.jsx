  ...
  const onDetail = (id) => {
    ...
    return(
        ...
        <div key={r.id} className=" cursor-pointer mt-5 border-2 hover:bg-main-beige border-main-green h-full pt-5 pb-5 text-center rounded-lg shadow-lg" onClick={() => onDetail(r.id)}> 
            {r.type === 'quote' ? (
              <>
                <div className=" pl-3 pb-3 text-sm text-start">{highlightKeyword(r.sourceTitle, input)} ({r.source_type})</div>
                <div className="text-lg">{highlightKeyword(r.content, input)}</div>
                <div className="text-sm text-gray-600">{highlightKeyword(r.creator, input)}</div>
              </>
            ) : (
              <>
                <div className=" pl-3 pb-3 text-sm text-start">{highlightKeyword(r.title, input)} ({r.source_type})</div>
